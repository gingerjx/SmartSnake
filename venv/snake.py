...
class Snake:
  def __init__(self, head_coords, board_size, cell_size, snake_color):
    self.eaten_fruits = 0
  ...
  def ate_fruit(self, coords):
    if coords == self.body[0]:
      self.eaten_fruits += 1
      return True
    else:
      return False
  ...
  def draw(self, board):
    for i in range(len(self.body)):
      mapped_coords = self.map_coords(self.body[i])
      (r, g, b) = self.snake_color
      r = (r + i*5) if (r + i*5) < 255 else 255
      segment_color = (r, g, b)
      pygame.draw.rect(board, segment_color, mapped_coords + self.cell_size)
